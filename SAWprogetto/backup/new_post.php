...
<?php
    ...
    $stmt = $con->prepare("INSERT INTO test.posts (title, filename, tags, poster) VALUES (?, ?, ?, ?)");
    ...
    //Prendo il valore id del post
    $stmt2 = $con->prepare("SELECT id FROM test.posts WHERE poster=? AND title=? AND filename=?");
    $stmt2->bind_param('sss', $poster, $title, $filename);
    $res = $stmt2->execute();
    ...
    {
        //return false;
        echo "Error fetching post from DB";
    }
    $stmt2->close();
   }
